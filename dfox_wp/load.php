...
if (!function_exists('dfox_wp_menu')) {
    if (is_multisite()) {
        add_action('network_admin_menu', 'dfox_wp_menu');
    } else {
    ...
    {
        $url = dfox_wp_admin_url('admin.php?page=dfox_wp_admin&plugin=' . $_GET['page']);
...
if (!function_exists('dfox_wp_admin_url')) {
    function dfox_wp_admin_url($path = '', $scheme = 'admin')
    {
        return is_multisite() ? network_admin_url($path, $scheme) : admin_url($path, $scheme);
    }
}
?>